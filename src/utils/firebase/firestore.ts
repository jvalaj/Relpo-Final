...
export async function addRide({name,ridersCount,city,uuid,host,participants}:RideInterface) {
    console.log(host)
    return await fire.firestore().collection('rides').add({name,ridersCount,city,uuid,host,participants});
